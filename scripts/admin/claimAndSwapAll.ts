...
async function main(): Promise<void> {
  ...
  if (fundAssets.length > 0) {
    ...
    if (fundAssets.length > 1) {
      for (let i = 1; i < fundAssets.length; i++) {
        apiUrl = `https://api.1inch.exchange/v3.0/1/swap?fromTokenAddress=${fundAssets[i]}&toTokenAddress=${xtkAddress}&amount=${feeBalances[i]}&fromAddress=${revenueControllerAddress}&slippage=1&disableEstimate=true`;
        response = await axios.get(apiUrl);
        calldata = response.data.tx.data;

        await revenueController.connect(manager).swapOnceClaimed(fundIndex, i, calldata);
      }
    }
  }
}
